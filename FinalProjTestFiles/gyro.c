...
uint32_t *readGyro(){
        ...
        readOut[0] = combine_bytes(accel[0], accel[1]) * 0.000122f;
        readOut[1] = combine_bytes(accel[2], accel[3]) * 0.000122f;
        readOut[2] = combine_bytes(accel[4], accel[5]) * 0.000122f;

        readOut[3]= combine_bytes(gyro[0], gyro[1])  *.030f;
        readOut[4] = combine_bytes(gyro[2], gyro[3]) *.030f;
        readOut[5]= combine_bytes(gyro[4], gyro[5])  *.030f;

        return readOut;
}
uint16_t readTemp(){
    return combine_bytes(temp[0], temp[1]);
}
