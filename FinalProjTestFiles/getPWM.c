...
uint32_t* get_duty2(uint16_t pin[]) {
    ...
    while (true) {
        ...
        if(timer_elapsed_us(t1, t2) >= readPeriod) {
            t1 = t2 = timer_read(); //reset count, log time before runtime
            if(gpio_get(pin[0]) == 1) on2[0]++;
            if(gpio_get(pin[1]) == 1) on2[1]++;
            if(gpio_get(pin[2]) == 1) on2[2]++;
            if(gpio_get(pin[3]) == 1) on2[3]++;
            if(gpio_get(pin[4]) == 1) on2[4]++;
            if(gpio_get(pin[5]) == 1) on2[5]++;
        }
        if (timer_elapsed_us(t3, t4) >= period_us) {
            //printf("On = %d \n", on);
            //percent = on-100;
            for(int i=0;i<6;i++) on2[i]=  ((on2[i]-100)); //returns 100-200 value, subtract 100 to normalize to percent value
            return on2; //returns a number array of values 0-100 
            //need to ensure all of these values are normalized to 0-100 values
            }
    }
}
